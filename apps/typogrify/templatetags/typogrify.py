...
def widont(text):
    ...
    """
    text = force_unicode(text)
    widont_finder = re.compile(r"""((?:</?(?:a|em|span|strong|i|b)[^>]*>)|[^<>\s]) # must be preceded by an approved inline opening or closing tag or a nontag/nonspace
...
def check_letters(first, second):
    if first in u'CKz' or first in u'КС': # lat, cyr
        return '0'
    if first in u'Г':
        return '-0.25'
    elif first in u'T' or first in u'Т':
        return '-0.15'
    elif first in u'FPYW' or first in u'РУ':
        if second in u'iktl' or second in u'ёйб' or second.upper() == second:
            return '-0.1'
        return '-0.15'
    return '-.05'

@register.filter
def title_tracking(value):
    '''
    Set tracking for first letter of phrase. Decides by checking
    first and second letters by known patterns.

    Thinks that first letter is very big (200%).
    '''
    first, second = value[:2]
    width = check_letters(first, second)
    first = (u'<span class="first" style="letter-spacing:'
             '%sem">%s</span>' % (width, first))
    return mark_safe(first + value[1:])
title_tracking.is_safe = True
